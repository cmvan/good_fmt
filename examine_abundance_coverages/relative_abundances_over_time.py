"""
File: relative_abudances_over_time.py
-------------------------------------
This program graphs changes in relative abundance for abundant species in a given sample from the 
Li et al. 2016 study. It also writes a file containing the relative abundances for all abundant
...
if __name__=='__main__':
    ...
    for idx in range(int(num_species)): # was num_species/2
        ...
        if rec_abundances[0] == donor_abundance == 0:
            continue
        elif np.sum(np.array(rec_abundances), dtype=np.float64) + donor_abundance < 1e-3: # Can adjust limit as desired
        #(FAT_020: 1e-4, FAT_006: 1e-3)
            continue
        else:
    ...
    output_file_path = os.path.join(os.getcwd(), 'midas_out/species_merges', '%s_merged_species' % 
                                    recipient_name, 'relative_abundances_over_time.txt')
    ...
    # Save the figure
    fig_path = os.path.join(os.getcwd(), 'figures', 'relative_abundances_at_timepoints', 
                            '10_29_21_Cut_3_%s_vs_%s_Timepoint_Relative_Abundances' % (donor_name, recipient_name))
    plt.savefig(fig_path, dpi=760)
